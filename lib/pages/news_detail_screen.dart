...
class _NewsDetailScreenState extends State<NewsDetailScreen> {  
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data['status'] == 'success' && data['results'] != null) {
          ...
          final newRelatedArticles = results
              .map<NewsArticle>((json) => NewsArticle.fromJson(json))
              .where((relatedArticle) => relatedArticle.id != widget.article.id) // Filter out the current article
              .toList();

          setState(() {
            _relatedArticles = newRelatedArticles;
            _relatedNextPageToken = nextPage; // Store the token for potential 'load more'
            _isLoadingRelated = false;
          });
        } else {
            print("Related API Error: ${data['results']?['message'] ?? data['message'] ?? 'Unknown API error'}");
            throw Exception('API returned error for related articles');
        }
      } else {
         print("Related HTTP Error: ${response.statusCode}, Body: ${response.body}");
         throw Exception('Failed to load related articles: HTTP ${response.statusCode}');
      }
    } catch (e, stackTrace) {
  ...
  Future<void> _fetchMoreRelatedArticles() async {
     ...
     try {
       final String? apiKey = dotenv.env['NEWSDATA_API_KEY'];
       if (apiKey == null || apiKey.isEmpty) throw Exception('API key not found');
       ...
       if (response.statusCode == 200) {
         ...
         if (data['status'] == 'success' && data['results'] != null) {
           ...
           final newRelatedArticles = results
               .map<NewsArticle>((json) => NewsArticle.fromJson(json))
               .where((relatedArticle) => relatedArticle.id != widget.article.id)
               .toList();
          ...
          throw Exception('Failed to load more related articles: HTTP ${response.statusCode}');
       }
     } catch (e) {
  ...
  Future<void> _launchURL(String url) async {
    ...
    try {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
           SnackBar(content: Text('Error opening the link.')),
        );
    }
  }
  ...
  Widget _buildSliverAppBar(BuildContext context) {    
     return SliverAppBar(
       ...
       backgroundColor: Colors.white,
       leading: IconButton(
         icon: Icon(Icons.arrow_back_ios_new, color: Colors.black87), // Use iOS style back arrow
       ...
       actions: [
         IconButton(
           icon: Icon(
             ...
             color: _isBookmarked ? Colors.blue[700] : Colors.black87, // Highlight if bookmarked
           ),
           tooltip: _isBookmarked ? 'Remove Bookmark' : 'Bookmark Article',
           onPressed: () {
             // TODO: Implement actual bookmark saving/removing logic
             setState(() {
           ...
           onPressed: () {
             // TODO: Implement share functionality (e.g., using share_plus package)
              if (widget.article.url.isNotEmpty) {
                 ...
                 ScaffoldMessenger.of(context).showSnackBar(
                   SnackBar(content: Text('Sharing functionality not fully implemented.')),
                 );
              } else {
                 ScaffoldMessenger.of(context).showSnackBar(
       ...
       child: Column(
         ...
         children: [
           ...
           Padding(
             ...
             child: Wrap(
               spacing: 8, // Spacing between chips if multiple categories were shown
               children: [
                 if (widget.article.category.isNotEmpty && widget.article.category != 'NEWS')
                   Chip(
                     label: Text(
           ...
           // Author (optional)
           if (widget.article.author.isNotEmpty && widget.article.author != 'Unknown')
             Padding(
               padding: const EdgeInsets.symmetric(horizontal: 16.0, vertical: 4.0),
               child: Row(
                 children: [
                   CircleAvatar(
                     ...
                     child: Icon(Icons.person_outline, size: 16, color: Colors.grey[600]),
                   ),
                   SizedBox(width: 8),
                   Text(
           ...
           if (widget.article.imageUrl.isNotEmpty)
             Container(
               ...
               margin: EdgeInsets.only(top: 16.0, bottom: 8.0), // Add vertical margin
               child: CachedNetworkImage(
  ...
  Widget _buildContent() {
     ...
     if (content.isEmpty || content.length < 20) { // Adjust length threshold if needed
         return Padding(
     ...
     return Padding(
       ...
       child: Column(
         crossAxisAlignment: CrossAxisAlignment.start,         
         children: _formatContentParagraphs(content).map((paragraph) =>
           Padding(
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            spacing: 8,
            runSpacing: 6,
            children: widget.article.countries.map((country) =>
              Chip(
                 label: Text(
                   country.toUpperCase(), // Ensure country codes/names are uppercase
                   style: TextStyle(
                      color: Colors.grey[800],
                      fontSize: 11,
                      fontWeight: FontWeight.w500
  ...
  Widget _buildRelatedNews() {
    return Column(
      ...
      children: [
        ...
        if (_isLoadingRelated && _relatedArticles.isEmpty) // Only show initial loading
          Padding(
        ...
        if (_relatedArticles.isNotEmpty)
          Container(
            ...
            child: ListView.builder(
              ...
              padding: EdgeInsets.symmetric(horizontal: 12), // Padding for the list itself
              itemCount: _relatedArticles.length + (_isLoadingRelated && _relatedNextPageToken != null ? 1 : 0), // Add space for loader when loading more
              itemBuilder: (context, index) {
                 if (index == _relatedArticles.length && _isLoadingRelated && _relatedNextPageToken != null) {
                ...
                return GestureDetector(
                  onTap: () {
                    ...
                    // Use pushReplacement if you don't want to stack detail screens infinitely
                     Navigator.pushReplacement( // or push
                      context,
                      MaterialPageRoute(
                  ...
                  child: Container(
                    ...
                    margin: EdgeInsets.symmetric(horizontal: 6), // Adjust spacing
                           ...
                           child: Container(
                             ...
                             child: article.imageUrl.isNotEmpty
                               ? CachedNetworkImage(
                                   ...
                                   errorWidget: (context, url, error) => Icon(Icons.article, color: Colors.grey[500], size: 30),
                                 )
                               : Center(child: Icon(Icons.article, color: Colors.grey[500], size: 30)), // Placeholder icon
                           ),
                        ),
                        // Content
                          ...
                          child: Padding(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                SizedBox(height: 4),
                                Text(
                                  article.category.toUpperCase(), // Show category
                                  style: TextStyle(
        ...
        if (!_isLoadingRelated && _relatedArticles.isEmpty && _errorMessage.isEmpty)
           Padding(
             padding: const EdgeInsets.symmetric(vertical: 20.0, horizontal: 16.0),
             child: Center(child: Text('No related articles found.', style: TextStyle(color: Colors.grey[600]))),
           ),
      ],
    );
  }
  ...
  String _formatDate(String dateString) {
    ...
    try {
      final DateTime? date = DateTime.tryParse(dateString); // Use tryParse for safety
      if (date != null) {
         ...
         if (difference.inDays == 0 && now.day == date.day) {
             ...
             return DateFormat('EEEE, MMM d').format(date); // e.g., Tuesday, Jun 11
         } else {
  ...
  List<String> _formatContentParagraphs(String content) {
    ...
    List<String> paragraphs = content
        ...
        .where((p) => p.isNotEmpty && p.length > 10) // Filter out very short lines/empty strings
        .toList();
    ...
    if (paragraphs.length <= 2 && content.length > 300) {
       ...
       for(String sentence in sentences) {
          if ((currentParagraph + sentence).length < 400 || currentParagraph.isEmpty) { 
